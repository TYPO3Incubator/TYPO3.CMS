...
class TBE_FolderTree extends localFolderTree {
	 ...
	 *
	 * @param	string		Title, ready for output.
	 * @param	t3lib_file_Folder The folderObject 'record'
	 * @return	string		Wrapping title string.
	 */
...
class browse_links {
	...
	function expandFolder(t3lib_file_Folder $folder, $extensionList='') {
		...
		if ($folder->checkActionPermission('browse')) {
			...
			$picon='<a href="#" onclick="return link_folder(\'file:'.t3lib_div::rawUrlEncodeFP($folder->getCombinedIdentifier()).'\');">' . $folderIcon . '</a>';
			...
			foreach ($items as $fileOrFolderObject) {
				...
				if ($renderFolders) {
					...
					$itemUid = 'file:' . $fileOrFolderObject->getUid();
				}

					// If the listed file turns out to be the CURRENT file, then show blinking arrow:
				if (($this->curUrlInfo['act'] == 'file' || $this->curUrlInfo['act'] == 'folder') && $currentIdentifier == $fileOrFolderObject->getCombinedIdentifier()) {
					$arrCol='<img'.t3lib_iconWorks::skinImg($GLOBALS['BACK_PATH'],'gfx/blinkarrow_left.gif','width="5" height="9"').' class="c-blinkArrowL" alt="" />';
				} else {
					$arrCol='';
				}
				...
				if (is_array($files)) {
					...
					foreach ($files as  $fileObject) {
						$fileInfo = $fileObject->getStorage()->getFileInfo($fileObject);
							...
							// URL of image:
						$iUrl = t3lib_div::rawurlencodeFP($fileObject->getPublicUrl());
