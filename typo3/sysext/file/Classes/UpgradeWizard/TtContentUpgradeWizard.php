	...
	protected function migrateField($table, $row, $fieldname, $fieldConfiguration) {
		...
		foreach ($fieldItems as $item) {
			...
			if(!PATH_site) throw new Exception('PATH_site was undefined.');
				...
				// copy file
			$sourcePath = PATH_site.'uploads/'.ltrim($fieldConfiguration['basePath'], '/') . $item;
			$destinationPath = PATH_site.'fileadmin/'.ltrim($fieldConfiguration['basePath'], '/') . $item;
			if(!is_dir(dirname($sourcePath))) {
				mkdir(dirname($sourcePath));
			}
			...
			copy($sourcePath, $destinationPath);
				...
				// get the File object
			$file = $this->storage->getFile($fieldConfiguration['basePath'] . $item);
