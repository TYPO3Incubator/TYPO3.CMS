...
class t3lib_file_Driver_LocalDriverTest extends t3lib_file_BaseTestCase {
	 ...
	 * @test
	 */
	public function getPublicUrlQueriesPublisherForUrlIfNoBaseUriIsSet() {
		$this->addToMount(array(
			'file.ext' => 'asdf',
			'subfolder' => array(
				'file2.ext' => 'asdf'
		...
		$mockedPublisher = $this->getMock('t3lib_file_Service_Publishing_FilesystemPublisher', array(), array(), '', FALSE);
		$mockedPublisher->expects($this->once())->method('publishFile')->with($this->equalTo($mockedFile1));
		$fixture->setResourcePublisher($mockedPublisher);

		$fixture->getPublicUrl($mockedFile1);
	}
	...
	public function foldersCanBeMovedWithinStorage() {
		...
		$fixture->moveFolderWithinStorage($sourceFolder, $mockedFolder);
	...
	public function foldersCanBeCopiedWithinSameStorage() {
		...
		$fixture->copyFolderWithinStorage($sourceFolder, $targetFolder);
